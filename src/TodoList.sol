...
contract TodoList {
    ...
    function pendTodoItem(uint256 index, uint256 pendingPeriod) public {
        require(
            todoItems[index].pendingContext.isPending == false,
            "The todo item is pending!"
        );
    ...
    function restoreTodoItem(uint256 index) public {
        ...
        uint256 actualPendingPeriod =
            block.timestamp - todoItems[index].pendingContext.time;
        require(
            actualPendingPeriod <= pendingPeriod, "You exccess pending period!"
        );
